...
subtest 'Test CheckOutItem with existing checkout' => sub {
    ...
    is(
        $dom->{NCIPMessage}->{CheckOutItemResponse}->{Problem}->{ProblemDetail}->{text},
    ...
    is(
        $dom->{NCIPMessage}->{CheckOutItemResponse}->{Problem}->{ProblemValue}->{text},
        $item_1->barcode,
        'CheckOutItemResponse returns correct problem barcode'
    );

    is(
        $dom->{NCIPMessage}->{CheckOutItemResponse}->{Problem}->{ProblemElement}->{text},
        'ItemIdentifierValue',
        'CheckOutItemResponse returns correct problem element'
    );

    is(
        $dom->{NCIPMessage}->{CheckOutItemResponse}->{Problem}->{ProblemType}->{text},
